...
users = [
  ...
  {
    email: 'some_other_person@gmail.com',
    password: 'password'
  }
]
...
dogs.each do |dog|
  ...
  dog = Dog.new(name: dog[:name], description: dog[:description], user_id: User.first.id)
  if dog.save
    print "#{dog[:name]} saved"
  else
    print "#{dog[:name]} did not save"
  end
